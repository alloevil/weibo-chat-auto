...
**含点/连字符的拉丁段额外拆段**：`github.com` 既保留整体，也产出 `github` + `com`。不这么做的话搜「github」匹配不到文档里的 `github.com` —— 正则把 `.` 收进词内，两者是不同 token，实测搜 `github`、`arxiv` 全部落空。保留整体是为了搜完整域名（`example.com`）时它作为精确 token 拿到更高 idf。

### 检索文本包含哪些字段

| 函数 | 内容 | 用途 |
|---|---|---|
| `msgBody(m)` | `content` + `share.title` + `share.url`/`link` + `videoUrl` + 「链接」「视频」「图片」标记 | `count_messages` 的词面匹配 |
| `msgText(m)` | `user` + `msgBody(m)` | BM25 文档文本 |

**为什么要把媒体字段纳入**：`normalize-message.js` 会把链接写进独立的 `share.url` / `link` / `videoUrl` 字段（查看器的「链接/视频」筛选器就靠它们），原先检索侧完全看不到 —— 实测问「上周分享过什么链接」`matchCount = 0`。同理 `[图片xN]` 原先只在展示层 `formatMessage` 里加，搜「图片」也是 0。现在附上中文词让模型用自然说法就能命中，不必猜域名。

**为什么 `count_messages` 不用 `msgText`**：`msgText` 含 `user`，那在 BM25 打分里是有效信号（问"tk 说了什么"时正文可能不含"tk"）；但计数若匹配人名，「搜张三」会把张三说的所有话都算进去，与独立的 `person` 参数职责重叠、让计数含义变模糊。有测试守着这条。

### 复读折叠

群聊里复读/转发极常见，而 BM25 的 tf 饱和**不足以**压制它。实测：

```
查询「半导体」的块级排序
  0.463  [纯复读块] 半导体还没跌到位 | 半导体还没跌到位 | 半导体还没跌到位 | 半导体还没跌到位
  0.250  [有信息块] tombkeeper: 我把手里的芯片股清了一半，半导体这轮估值还没消化完
```

纯复读排在有实际信息的块之前，模型先读到的是同一句话的 N 个副本（端到端实测 59% 的 snippet 行是重复内容）。`collapseRepeats` 在片段渲染时折叠连续重复，标注 `(连续重复 N 次)`。

两个刻意的选择：

- **只折叠连续重复，不全局去重**。相隔很远的同一句话可能是不同语境下的独立发言，合并会丢时间线。
- **保留首条 + 次数标记，不静默删除**。复读本身有信息量（刷屏、附和的强度），模型该知道发生过，只是不必读 N 遍。
- 空内容消息（纯图片/纯分享）不参与折叠 —— 它们的区别在媒体字段上，按 `content` 归一会把不同的分享合成一条。
