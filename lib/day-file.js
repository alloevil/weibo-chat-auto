...
/**
 * 同 id 记录合并：既有记录的非空字段优先，缺失字段从新记录补齐。
 *
 * 为什么不能让新记录无条件覆盖：归档器有三条消息来源，Puppeteer 网络层
 * 抓到的记录只有 8 个基础字段（无 pics/share/link/videoUrl/avatar）。页面
 * 加载时的 query_messages 流量会把上一轮已归档的富记录重新"降级"捕获一遍，
 * 覆盖即字段凭空消失。字段级合并后：降级记录补不了任何东西，富记录永存；
 * 同时新来源带来的额外字段仍能落盘。
 */
...
function mergeIntoDayFile(dayFile, newMsgs) {
    ...
    for (const m of newMsgs) {
        const prev = merged.get(String(m.id));
        merged.set(String(m.id), prev ? mergeRecords(prev, m) : m);
    }
