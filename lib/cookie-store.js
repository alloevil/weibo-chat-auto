...
const COOKIE_FILE = process.env.WEIBO_COOKIE_FILE
    ? path.resolve(process.env.WEIBO_COOKIE_FILE)
    : path.join(__dirname, '..', 'cookies.json');

/**
 * 落盘 Cookie 文件，权限收紧为 0600。
 * cookies.json 含 SUB 等完整登录态，默认 0644 会让多用户机器上的
 * 其他本地用户直接读走会话。writeFileSync 的 mode 只在创建时生效，
 * 覆盖已存在的文件不会改权限，所以再补一次 chmod。
 */
function writeCookieFile(file, cookies) {
    // 原子写（tmp + rename）：截断写在进程被杀/磁盘满时留下半截 JSON，
    // loadCookies 解析失败返回 []，下轮归档直接报「Cookie 已失效」要求重新扫码。
    // 日文件与归档状态早就是原子写，唯一最贵的文件不该走唯一的非原子路径。
    const tmp = `${file}.tmp-${process.pid}`;
    try {
        fs.writeFileSync(tmp, JSON.stringify(cookies, null, 2), { mode: 0o600 });
...
function saveCookies(cookies, reason = '') {
    ...
    writeCookieFile(COOKIE_FILE, cookies);
    console.log(
        `[cookie-store] 已保存 ${cookies.length} 个 Cookie${reason ? `（${reason}）` : ''}`
    );
...
function absorbSetCookies(setCookieLines, requestUrl, reason = 'Set-Cookie 续期') {
    if (!Array.isArray(setCookieLines) || setCookieLines.length === 0)
        return { ok: true, changed: 0 };
    let requestHost = '';
    try {
        requestHost = new URL(requestUrl).hostname;
    } catch {
        /* 无效 URL 则依赖 Domain 属性 */
    }
