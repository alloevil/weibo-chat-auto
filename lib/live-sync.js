...
async function fetchRecent({
    ...
    const url =
        `${API_ORIGIN}/webim/groupchat/query_messages.json` +
...
async function pollGroupOnce(group, deps) {
    ...
    for (let page = 0; page < MAX_BACKFILL_PAGES; page++) {
        ...
        const fresh = messages.filter(
            (m) => !group.seen.has(String(m.id)) && !roundIds.has(String(m.id))
        );
 ...
 * @param {() => boolean} [opts.isLocked] 归档器互斥锁探测（state/sync.lock）。
 *   归档器运行期间跳过整轮：它与归档器对同一日文件做"读→合并→写"，跨进程
 *   无互斥，撞上会按旧快照写回、覆盖掉对方刚落盘的消息（live-sync 的 seen
 *   已含该消息，覆盖即永久丢失）。跳过不推进游标，下轮自然补回。
 */
function createLiveSync({
    ...
    function groupsForPoll() {
        ...
        for (const g of resolveGroups()) {
            ...
            s.dir = g.dir;
            out.push({ name: g.name, displayName: g.displayName, ...s });
        }
        return out;
    }

    async function runRound() {
        // 归档器正在跑：跳过整轮（理由见 createLiveSync 的 isLocked 文档）
        if (isLocked()) {
            log('[live] 归档器正在运行，跳过本轮');
            return;
        }
        for (const g of groupsForPoll()) {
            try {
                ...
                if (r.newMessages.length > 0) {
                    log(`[live] ${g.displayName || g.name}: +${r.newMessages.length} 条`);
                    emit({
                        type: 'messages',
                        group: g.name,
                        groupLabel: g.displayName || g.name,
                        dates: r.dates,
                        messages: r.newMessages,
                    });
                }
            } catch (e) {
    ...
    return {
        ...
        refresh() {
            if (isEnabled() && subscribers > 0) start();
            else stop();
        },
        get subscribers() {
            return subscribers;
