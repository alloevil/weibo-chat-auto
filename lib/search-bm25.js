...
//
// 含点的拉丁段（域名、文件名）额外拆出各段：`github.com` 既保留整体，也产出
// `github` 与 `com`。不这么做的话搜「github」匹配不到文档里的 `github.com`
// ——正则把 `.` 收进词内，两者是不同 token。保留整体是为了让搜完整域名
// (`example.com`) 时它作为一个精确 token 拿到更高的 idf。
    ...
    while ((m = re.exec(text)) !== null) {
        if (m[1]) {
            ...
            if (seg.includes('.') || seg.includes('-')) {
                // 纯标点段（"..." "--"）拆不出东西，filter 兜住
                for (const part of seg.split(/[.-]+/)) {
                    if (part && part !== seg) tokens.push(part);
                }
            }
        }
    }
