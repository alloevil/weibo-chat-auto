 ...
 * @param {string|number|null} [deps.startMaxMid]
 *        续传游标：从该 mid 继续往下翻（而不是从最新一页重来）。上一轮撞
 *        MAX_PAGES/限流被打断时，归档器把它存进 state，下轮续传 —— 否则
 *        积压超过 maxPages×pageSize 条时每轮都翻同样的前 N 页、永远走不完。
 * @param {number} [deps.maxPages]
 * @param {number} [deps.pageSize]
 * @param {number} [deps.pageDelayMs]
 * @returns {Promise<{messages: object[], paginationComplete: boolean, paginationNote: string, cursorMaxMid: string|null}>}
 */
async function paginateMessages({
    ...
    let maxMid = startMaxMid !== null && startMaxMid !== undefined ? String(startMaxMid) : null;
    let pageNum = 0;
    ...
    while (pageNum < maxPages) {
        ...
        // 时间截止
        const pageOldestTs =
            typeof firstMsg?.time === 'number' && firstMsg.time > 0 ? firstMsg.time * 1000 : now();
        if (stopTimestamp > 0 && pageOldestTs < stopTimestamp) {
            log(
                `[API] 到达截止时间，停止 (消息时间=${new Date(pageOldestTs).toLocaleString('zh-CN')})`
            );
            paginationComplete = true;
            break;
        }
    ...
    // cursorMaxMid：已翻到的最老边界。分页走完时无意义（归档器只在残缺时
    // 持久化它作为下轮续传点）。
    return { messages, paginationComplete, paginationNote, cursorMaxMid: maxMid };
}
