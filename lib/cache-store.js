...
/**
 * 为流式代理保留一个有硬上限的缓存副本。
 * 一旦超过上限立即释放已经积累的块，后续 push 只返回 false、不再占用更多内存。
 */
function createBoundedBuffer(limit = MAX_ENTRY_BYTES) {
    let chunks = [];
    let byteLength = 0;
    let exceeded = false;
    return {
        push(chunk) {
            ...
            if (!isCacheable(byteLength, limit)) {
                chunks = [];
                exceeded = true;
                return false;
            }
        ...
        toBuffer() {
            return exceeded || !isCacheable(byteLength, limit)
                ? null
