...
function createNormalizeMessage({ formatDate, formatTime }) {
    return function normalizeMessage(m) {
        ...
        if (!id) return null;
        // 时间戳：time（秒）优先，其次 created_at，兜底 Date.now()。
        // 兜底记录必须打 tsEstimated —— 它不能参与归档状态推进：分页从新到旧
        // 倒着走，一条"当前时刻"的估算时间戳会把 lastTimestamp 推到 now，
        // 分页起点与它之间尚未拉到的消息下轮被 stopTimestamp 跳过（永久空洞）。
        // created_at 解析出 NaN 同样按估算处理，否则会产出 weibo_chat_NaN-NaN-NaN.json。
        let ts =
            typeof m.time === 'number' && m.time > 0
                ...
                : m.created_at
                  ...
                  : NaN;
        const tsEstimated = !Number.isFinite(ts);
        if (tsEstimated) ts = Date.now();
        ...
        if (tsEstimated) result.tsEstimated = true;
