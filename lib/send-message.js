...
const UPLOAD_PATH = '/webim/uploadx.json';
 ...
 */
async function sendGroupMessage({ groupId, content, cookieHeader, fetchImpl = fetch, timeoutMs = 15000, mediaType = '0', fids = '' }) {
    ...
    if (fids) params.fids = String(fids);   // 图片/文件消息带附件 id
 ...
 * 上传图片并发到群里。
 *
 * 两步（端点取自 webim bundle）：
 *   1. POST /webim/uploadx.json（multipart）→ 返回 fid
 *   2. send_message.json 带 fids + media_type=1，content 固定"分享图片"
 *      —— 与真实客户端一致（它也发这个文案），且必须同样带 annotations，
 *      否则风控会静默撤回（见文件头注释）。
 * @returns {{ok: boolean, needLogin?: boolean, error?: string, fid?: string, messageId?: string}}
 */
async function sendGroupImage({ groupId, buffer, filename = 'image.png', mimeType = 'image/png', cookieHeader, fetchImpl = fetch, timeoutMs = 60000 }) {
    if (!groupId) return { ok: false, error: '缺少群会话 id（需先跑一次归档让它被记录）' };
    if (!buffer || !buffer.length) return { ok: false, error: '图片内容为空' };
    if (buffer.length > MAX_IMAGE_BYTES) {
        return { ok: false, error: `图片过大（${(buffer.length / 1048576).toFixed(1)}MB > ${MAX_IMAGE_BYTES / 1048576}MB）` };
    }
    ...
    let up;
    try {
        const resp = await fetchImpl(`${API_ORIGIN}${UPLOAD_PATH}?source=209678993`, {
            method: 'POST',
            headers: {
                Cookie: cookieHeader,
                Referer: `${API_ORIGIN}/chat`,
            ...
            body: form,   // 不手写 Content-Type：boundary 由 fetch 生成
            signal: AbortSignal.timeout(timeoutMs),
        });
        up = await resp.json();
    } catch (e) {
        return { ok: false, error: `图片上传失败: ${e.message}` };
    }

    if (up.error_code === UNAUTHENTICATED_CODE) {
        return { ok: false, needLogin: true, code: up.error_code, error: '微博 Cookie 已失效，请重新扫码登录' };
    }
    const fid = up.fid || up.fids || up.data?.fid || null;
    if (!fid) return { ok: false, error: `上传未返回 fid（${JSON.stringify(up).slice(0, 120)}）` };

    return sendGroupMessage({
        groupId, cookieHeader, fetchImpl, timeoutMs,
        content: '分享图片',
        mediaType: '1',
        fids: String(fid),
    }).then(r => (r.ok ? { ...r, fid: String(fid) } : r));
}

module.exports = {
    sendGroupMessage, sendGroupImage,
    MAX_CONTENT, MAX_IMAGE_BYTES, SEND_PATH, UPLOAD_PATH,
    UNAUTHENTICATED_CODE, GROUP_NOT_FOUND_CODE,
};
