...
(function (root, factory) {
    ...
    /**
     * URL → 可安全放进 HTML 属性（src/href）的字符串；不安全返回 ''。
     * 归档数据里的 URL 来自远端（消息、分享卡、头像），任何一次污染都是持久
     * XSS：scheme 白名单挡 javascript:/data:，escapeAttr 挡属性逃逸。
     * 放行 http(s) 绝对地址与站内相对路径（/api/image、/api/sinaimg 代理）。
     */
    function safeAttrUrl(u) {
        const s = String(u == null ? '' : u).trim();
        if (!s) return '';
        if (/^https?:\/\//i.test(s) || s.startsWith('/')) return escapeAttr(s);
        return '';
    }
