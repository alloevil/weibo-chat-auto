...
const SESSIONS_PATH =
    '/webim/2/direct_messages/contacts.json' + '?special_source=3&source=209678993';
...
function parseGroupSessions(json) {
    const list = Array.isArray(json?.contacts)
        ? json.contacts
...
async function fetchGroupSessions({ cookieHeader, fetchImpl = fetch, timeoutMs = 15000 } = {}) {
    ...
    if (!groups.length) {
        return {
            ok: false,
            error: '未解析出任何群会话（接口结构可能已变化，可继续手工编辑 config.json）',
        };
    }
