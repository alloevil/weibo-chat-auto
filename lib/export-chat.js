...
// /api/messages 会就地改写缓存里的 pics 为本地代理路径；导出文件在别人
// 机器上打不开代理，这里还原成原始 URL（与 rewriteImageUrls 互逆）。
function originalPicUrl(u) {
    u = String(u || '');
    if (u.startsWith('/api/image?fid=')) {
        return `https://upload.api.weibo.com/2/mss/msget?source=209678993&fid=${u.split('fid=')[1]}`;
    }
    if (u.startsWith('/api/sinaimg?url=')) {
        try { return decodeURIComponent(u.slice('/api/sinaimg?url='.length)); } catch { return u; }
    }
    return u;
}
...
function renderMarkdown(messages, { group = '', date = '' } = {}) {
    ...
    for (const m of messages) {
        ...
        for (const pic of m.pics || []) lines.push(`![图片](${originalPicUrl(pic)})`);
    ...
    for (const m of messages) {
        ...
        if (m.pics && m.pics.length > 0) {
            attach += '<div class="msg-pics">' + m.pics.map(u => {
                const orig = escapeHtml(originalPicUrl(u));
...
module.exports = { parseForward, buildContentIndex, originalPicUrl, filterMessages, renderMarkdown, renderHtml };
