...
const raw = (id, timeSec) => ({
    id,
    time: timeSec,
    from_user: { screen_name: 'u' + id },
    content: 'c' + id,
});
...
function fakeFetch(pages) {
    ...
    return {
        ...
        fetchPage: async (url) => {
            ...
            const idx =
                maxMid === '0' ? 0 : pages.findIndex((p) => String(p[0]?.id) === maxMid) + 1;
...
test('中断后续传：从 state 的 stopTimestamp 恢复，到达截止即停且不翻后页', async () => {
    ...
    const pages = [[raw(30, 3000), raw(40, 4000)], [raw(20, 2000)], [raw(10, 1000)]];
    const { fetchPage, calls } = fakeFetch(pages);
    const r = await paginateMessages({
        groupId: '123',
        stopTimestamp: 2500 * 1000,
        fetchPage,
        normalize: normalizeMessage,
        sleep: noSleep,
        log: quiet,
...
test('截止边界：页面最旧消息恰等于 stopTimestamp 时不算越过，继续翻页', async () => {
    const pages = [[raw(20, 2000), raw(30, 3000)], []];
    const { fetchPage, calls } = fakeFetch(pages);
    const r = await paginateMessages({
        groupId: '123',
        stopTimestamp: 2000 * 1000,
        fetchPage, // pageOldestTs === stopTimestamp
        normalize: normalizeMessage,
        sleep: noSleep,
        log: quiet,
    });
...
test('接口 error_code：识别为失败而不是"无更多消息"', async () => {
    const fetchPage = async () => ({
        status: 200,
        data: { error_code: 21332, error: 'group not found' },
    });
    const r = await paginateMessages({
        groupId: '123',
        stopTimestamp: 0,
...
test('onPage 落盘回调：每页收到本页新增（去重后）的消息', async () => {
    const pages = [[raw(30, 3000), raw(40, 4000)], [raw(10, 1000), raw(30, 3000)], []];
    const { fetchPage } = fakeFetch(pages);
    const batches = [];
