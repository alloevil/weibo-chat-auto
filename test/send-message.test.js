...
test('sendGroupMessage: 缺 groupId / 空内容 / 超长内容不发请求', async () => {
    ...
    const impl = async () => {
        called++;
        return { status: 200, json: async () => ({ result: true }) };
    };

    assert.match(
        (
            await sm.sendGroupMessage({
                groupId: '',
                content: 'x',
                cookieHeader: 'c',
                fetchImpl: impl,
    ...
    assert.match(
        (
            await sm.sendGroupMessage({
                groupId: '1',
                content: 'x'.repeat(sm.MAX_CONTENT + 1),
                cookieHeader: 'c',
                fetchImpl: impl,
            })
        ).error,
...
test('sendGroupImage: 先 multipart 上传取 fid，再带 fids + media_type=1 发送', async () => {
    ...
    assert.ok(
        !('Content-Type' in up.init.headers),
        'boundary 交给 fetch 生成，不能手写 Content-Type'
    );
    ...
    assert.deepStrictEqual(
        JSON.parse(params.get('annotations')),
        { webchat: 1, clientid: '' },
...
test('sendGroupImage: 上传未返回 fid 时不发消息', async () => {
    const impl = uploadThenSend({
        upload: { result: false, error: '存储异常' },
        send: { result: true },
    });
    const r = await sm.sendGroupImage({
        groupId: '1',
...
test('sendGroupImage: 空图/超大图/缺 groupId 本地即拦，不打接口', async () => {
    ...
    assert.match(
        (
            await sm.sendGroupImage({
                groupId: '1',
                buffer: Buffer.alloc(sm.MAX_IMAGE_BYTES + 1),
                cookieHeader: 'c',
                fetchImpl: impl,
            })
        ).error,
