...
test('parseSetCookie 解析属性并换算 expires', () => {
    const c = cs.parseSetCookie(
        'ALF=02X9; Domain=.weibo.com; Path=/; Max-Age=3600; Secure; HttpOnly',
        'api.weibo.com'
    );
...
test('parseSetCookie Max-Age 优先于 Expires', () => {
    const c = cs.parseSetCookie(
        'X=1; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Max-Age=100; Domain=.weibo.com',
        'weibo.com'
    );
...
test('absorbSetCookies 跳过删除指令与无效输入（不落盘）', () => {
    ...
    assert.deepStrictEqual(cs.absorbSetCookies([], 'https://api.weibo.com'), {
        ok: true,
        changed: 0,
    });
    assert.deepStrictEqual(cs.absorbSetCookies(null, 'https://api.weibo.com'), {
        ok: true,
        changed: 0,
    });
});
