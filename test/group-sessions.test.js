...
const {
    parseGroupSessions,
    normalizeSession,
    diffConfiguredGroups,
    fetchGroupSessions,
} = require('../lib/group-sessions.js');

test('normalizeSession: 内嵌 group 形态（形态 A）', () => {
    const g = normalizeSession({
        type: 'group',
        group: { id: 123, name: '茧房建筑师协会', avatar: 'http://a/x.jpg', member_count: 42 },
    });
...
test('normalizeSession: 平铺形态（形态 B）与残缺条目', () => {
    const g = normalizeSession({
        ...
        id: 9,
        name: '群B',
        profile_image_url: 'http://a/b.jpg',
    });
...
test('fetchGroupSessions: 其它错误码与网络失败 → ok:false 且信息可读', async () => {
    ...
    const r2 = await fetchGroupSessions({
        fetchImpl: async () => {
...
test('fetchGroupSessions: 响应正常但解析不出群 → 明确指向手编 config.json 的老路', async () => {
    const r = await fetchGroupSessions({
        fetchImpl: async () => ({
            json: async () => ({ sessions: [{ type: 'private', id: 1, name: '某人' }] }),
        }),
    });
    assert.strictEqual(r.ok, false);
...
test('fetchGroupSessions: 正常路径返回群列表', async () => {
    const r = await fetchGroupSessions({
        fetchImpl: async () => ({
            json: async () => ({
                sessions: [{ type: 'group', group: { id: 7, name: '群X', avatar: 'http://a' } }],
            }),
        }),
    });
...
const realGroup = (id, name, groupType = 1, memberCount = 0) => ({
    ...
    user: {
        ...
        profile_image_url: `http://a/${id}.jpg`,
        avatar_large: `http://a/${id}_big.jpg`,
        members: [],
        admins: [],
...
test('parseGroupSessions: contacts 形态只挑出群，单聊一个都不许混入', () => {
    ...
    assert.deepStrictEqual(
        r.map((g) => g.name),
        ['茧房建筑师协会', '猫咪AI研究', '赛博动物园w'],
        '单聊混进选群面板 = 用户勾了之后归档器永远找不到那个群'
    );
    assert.strictEqual(
        r[0].id,
        '4761715839862414',
        '会话 id 必须原样保留（与 state.groupId 同源）'
    );
...
test('normalizeSession: user.type 非 2 一律不是群，哪怕带了头像等群同名字段', () => {
    ...
    assert.strictEqual(
        normalizeSession({ user: { id: 1, name: '伪群', type: 0, member_count: 9 } }),
        null
    );
