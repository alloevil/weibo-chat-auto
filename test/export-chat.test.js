...
const messages = [
    ...
    },
    {
        ...
        user: '李四',
        timestamp: 2000,
        time: '2026/07/01 09:06:30',
        date: '2026-07-01',
        content: `「早上好，今天开会」${SEP}收到，几点？`,
    },
    {
        id: 3,
        user: '王五',
        timestamp: 3000,
        time: '2026/07/01 09:07:00',
        date: '2026-07-01',
        content: '[doge] 我也来 [未知表情]',
    },
    {
        id: 4,
        user: '张三',
        timestamp: 4000,
        time: '2026/07/01 09:08:00',
        date: '2026-07-01',
        content: '看图',
        pics: ['https://upload.api.weibo.com/2/mss/msget?source=209678993&fid=12345'],
    },
    {
        id: 5,
        user: 'bot',
        timestamp: 5000,
        time: '2026/07/01 09:09:00',
        date: '2026-07-01',
        content: '收到红包消息',
    },
];
...
test('filterMessages: 默认滤掉红包/噪音，keepNoise 保留', () => {
    assert.deepStrictEqual(
        ex.filterMessages(messages).map((m) => m.id),
        [1, 2, 3, 4]
    );
...
test('renderMarkdown: 时间戳/昵称/引用 blockquote/图片 URL', () => {
    const md = ex.renderMarkdown(ex.filterMessages(messages), {
        group: '测试群',
        date: '2026-07-01',
    });
    ...
    // 图片输出原始 URL
    assert.ok(
        md.includes('![图片](https://upload.api.weibo.com/2/mss/msget?source=209678993&fid=12345)')
    );
    // 默认过滤了红包
...
test('renderHtml: 自包含（无外部 css/js 引用）、引用带原作者、表情不丢', () => {
    const html = ex.renderHtml(ex.filterMessages(messages), {
        group: '测试群',
        date: '2026-07-01',
    });
    // 自包含：无外部引用，样式内联
    ...
    assert.ok(
        !/src="https?:\/\/(?!upload\.api\.weibo\.com|wx\d\.sinaimg\.cn)/.test(html),
        '除消息图片外不得引用外部资源'
    );
