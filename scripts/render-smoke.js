...
for (const group of groups) {
    ...
    for (const date of targets) {
        ...
        try {
            ...
            vm.runInContext(
                fs.readFileSync(path.join(ROOT, 'lib/text-utils.js'), 'utf-8'),
                sandbox,
            ...
            sandbox.__msgs = msgs;
            vm.runInContext(
                `allMessages = __msgs; selectedDate = ${JSON.stringify(date)};`,
                sandbox
            );
            if (process.argv.includes('--no-noise-filter'))
                vm.runInContext('hideNoise = false', sandbox);
            vm.runInContext('renderAll()', sandbox);
            const len = vm.runInContext(
                "document.getElementById('messages').innerHTML.length",
                sandbox
            );
            if (msgs.length && len < 100)
                throw new Error(`渲染结果异常空(html ${len} 字,消息 ${msgs.length} 条)`);
        } catch (e) {
            failures++;
