...
const CONFIG = {
    ...
    chromePath: resolveChromePath(
        (() => {
            try {
                return require('../config.json').chromePath;
            } catch {
                return '';
            }
        })()
    ...
    try {
        ...
        if (needLogin) {
            ...
            cookieStore.saveCookies(
                cookieStore.filterWeiboCookies(await browser.cookies()),
                '归档运行续期'
            );
        }
        ...
        const finalCheck = await page
            .evaluate(() => !!window.__ARCHIVER_STATE__)
            .catch(() => false);
        console.log('归档脚本状态:', finalCheck ? '✓ 已就绪' : '✗ 未就绪');

        // 等待页面初始加载的 API 请求，获取第一个群的 group ID
        await delay(3000);
        let initialGroupId = await page.evaluate(
            () => window.__ARCHIVER_STATE__?.lastGroupId || null
        );
        ...
        for (const [groupIdx, currentGroupName] of GROUPS.entries()) {
            ...
            for (let attempt = 0; attempt <= GROUP_RETRIES && !groupDone; attempt++) {
                try {
                    if (attempt > 0) {
                        console.log(
                            `↻ 重试「${currentGroupName}」（第 ${attempt + 1}/${GROUP_RETRIES + 1} 次）`
                        );
                    ...
                    const groupClicked = await page.evaluate((groupName) => {
                        ...
                        for (const el of allElements) {
                            ...
                            if (
                                ...
                                text.length < groupName.length + 20 &&
                                el.offsetHeight > 0
                            ) {
                                el.click();
                    ...
                    if (groupClicked.found) {
                        ...
                        // 群 id 解析不到会在下面跳过此群。
                        console.log(
                            `⚠ 未找到群聊 "${currentGroupName}"（headless 无窗口，不等待手动操作）`
                        );
                    } else {
                    ...
                    if (groupId) {
                        ...
                        if (groupId)
                            console.log(`✓ 从页面加载时的 API 请求获取群组 ID: ${groupId}`);
                    }

                    if (!groupId) {
                        console.log(
                            `⚠ 无法获取群 "${currentGroupName}" 的 ID（切群后未捕获到 API 请求）`
                        );
                    ...
                    if (archivedGroupIds.has(groupId)) {
                        console.log(
                            `⚠ 群 "${currentGroupName}" 解析出的 ID ${groupId} 已被 "${archivedGroupIds.get(groupId)}" 占用，` +
                    ...
                    while (waitCount < 10) {
                        const count = await page.evaluate(
                            () => window.__ARCHIVER_STATE__?.getCount() || 0
                        );
                        if (count > 0) {
                    ...
                    async function fetchPage(url) {
                        ...
                        for (let attempt = 0; attempt < 4; attempt++) {
                            if (attempt > 0)
                                await delay(Math.min(2000 * 2 ** (attempt - 1), 16000));
                            try {
                                const resp = await httpsGet(url);
                                if (RATE_LIMITED.has(resp.status) || resp.status >= 500) {
                                    lastErr = new Error(`HTTP ${resp.status}`);
                                    console.log(
                                        `[API] ${RATE_LIMITED.has(resp.status) ? '被限流' : '服务端错误'} ${resp.status}，退避重试`
                                    );
                                    continue;
                                }
                    ...
                    console.log(
                        `API 分页获取完成: ${allApiMessages.length} 条消息` +
                    ...
                    if (messages.length > 0) {
                        ...
                        for (const [date, msgs] of Object.entries(groups)) {
                            ...
                            console.log(
                                `  ${date}: 原有 ${existing} + 本轮 ${msgs.length} → ${total} 条`
                            );
                        }
                        ...
                        if (!paginationComplete) {
                            ...
                            console.log(
                                `归档状态已保存 (截止: ${new Date(newState.lastTimestamp).toLocaleString('zh-CN')})`
                            );
                        }
