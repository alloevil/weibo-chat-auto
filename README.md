...
> **读长聊天记录的几个利器**：每条消息头部有 **🎯 上下文** 链接，点开右侧面板可看到它引用的原消息、前后对话邻域、以及后续谁回复了它；引用气泡标出原作者并可点击跳转；回到查看器时有「以下为新消息」分隔线。
...
| 必需 | 说明 |
| --- | --- |
| 🖥 **macOS / Linux / WSL** | 归档与查看器跨平台运行；定时任务自动安装仅 macOS（Linux 见下方 cron 说明） |
| 🟢 **Node.js 18+** | [brew install node](https://brew.sh)（macOS）/ `apt install nodejs`（Linux）/ [nodejs.org](https://nodejs.org) |
| 🌐 **Google Chrome** | 归档器用它登录并抓取消息；路径自动探测，无需手动配置 |
| 📱 **微博账号 + 手机 App** | 首次需用 App 扫码登录网页版 |

> Windows 用户请在 [WSL](https://learn.microsoft.com/windows/wsl/install) 中使用（等同 Linux 环境）。
...
> 💡 Puppeteer 启动的独立浏览器不共享日常 Chrome 的登录态，所以需要单独扫码一次。
...
> `chromePath` 留空即自动探测系统中的 Chrome（macOS / Linux / WSL）；仅在 Chrome 装在非默认位置时才需手动填写。
...
> 启用后定时归档会顺带刷新 Cookie，基本不会过期。日志在 `logs/archive.log`。不想配定时也行——随时 `npm run archive` 或在查看器点 Sync Now。
...
> 定时任务的 plist 由 `setup.sh` 在 `~/Library/LaunchAgents/` 下动态生成（含本机的 node 路径与项目目录）。
