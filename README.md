...
| 归档 | 查看器 |
| --- | --- |
| 🗂 多群支持（`config.json` 配置） | ⚡ **实时同步**：新消息自动出现（SSE 推送，无需点 Sync） |
| 🖥 原生桌面应用（应用内扫码登录） | 💬 **群内发言**：直接在查看器里发消息 |
...
查看器打开时，导航栏出现绿色 **实时** 指示灯：服务端每 20 秒拉一次新消息，并入日文件后通过 SSE 推给页面，新消息自动出现在列表里 —— 不用点 Sync Now（那仍是"补全历史"用的全量归档）。消息区底部的输入框可直接发言（Enter 发送，Shift+Enter 换行）。

两者都依赖**群会话 id**。归档器点击切群时会解析它并写进 `state/last-archive-state_<群名>.json`，所以：
