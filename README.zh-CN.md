...
如果配置缺失或不完整，AI 入口仍会显示；点击后会自动打开设置、列出缺失字段，并聚焦第一个待填写项。API Key 无效、额度限制、请求超时和服务地址不可达也会显示对应的可操作提示。

### Q&A 问答

点击工具栏的“问群历史”后输入问题，支持自然语言时间（“最近”、“昨天”、“上周”）和人名筛选。
...
索引是**自动的**：归档跑完会在后台为当天新增的话题块生成摘要与同义改写（只标新块，历史块按内容指纹复用，稳态下每小时约 1 次 LLM 调用）。没配 `ai-config.json` 也能用，只是少了这层跨词汇的召回。只有两种情况需要手动跑：给已有的历史存量补索引，或想重建全部索引 —— `node scripts/build-qa-index.mjs --group <群名> --all`（可断点续传，重跑自动跳过已完成的；存量大时先加 `--dry-run` 看看规模，或用 `--max-llm-calls N` 限量）。
